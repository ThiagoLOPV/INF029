...
#include "ThiagoVieira20212160044.h" // Substitua pelo seu arquivo de header renomeado
#include <stdio.h>
#include <stdlib.h>
/*
...
int q1(char data[]) {
  ...
  if (dq.iMes == 2) {
    ...
    if (dq.iDia > 29 || (dq.iDia == 29 && !bissexto)) {
      return 0;
    }
  }
  ...
  if (q1(datainicial) == 0) {
    ...
    if (dqfinal.iAno < dqinicial.iAno) {
      ...
      return dma;
               ...
               dqfinal.iMes < dqinicial.iMes) {
      dma.retorno = 4;
      return dma;
    } else if (dqfinal.iAno == dqinicial.iAno &&
               dqfinal.iMes == dqinicial.iMes &&
               dqfinal.iDia < dqinicial.iDia) {
      dma.retorno = 4;
      return dma;
    }
    ...
    if (dias < 0) {
      meses--;
      dias += diasDoMes(dqinicial.iMes, dqinicial.iAno);
    }
...
int q4(char *strTexto, char *strBusca, int posicoes[30]) {
  int i, j, k, count = 0, lenTexto = strlen(strTexto),
               lenBusca = strlen(strBusca);

  for (i = 0; i < lenTexto; i++) {
    for (j = i, k = 0; strTexto[j] == strBusca[k] && k < lenBusca; j++, k++)
      ;

    if (k == lenBusca) {
      posicoes[count * 2] = i + 1;
      posicoes[count * 2 + 1] = j;
      count++;
    }
  }
